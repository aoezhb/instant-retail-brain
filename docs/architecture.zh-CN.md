...
`business_unit_id`、`store_id`、`node_id` 始终分离。运行时的每个商品键都包含这三个身份以及 `sku_id`，因此不同经营主体使用相同局部 ID 时不会发生碰撞。Handler 还会生成明确的 `StoreNodeBinding`。

为了让示例保持简单，当前数据集按照已经归属到门店-仓点服务关系的粒度保存需求和库存。真实网络中的共享仓点库存、拆单履约或动态寻源，需要在客户项目中改成实际的分配和网络状态处理。
